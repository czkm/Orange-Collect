<template>
  <div class="rocker-launch">
    <a @click="handleClick" class="rocket-button" id="rocket-button">
      ...
      <div class="animation">
        <div class="rocket" :class="{'isSuccess':rockactive}">
          <svg>
...
export default {
  ...
  data() {
    return {
      rockactive: this.active
    };
  },
  watch: {
    active() {
      // console.log("in", this.active);
      if (this.active) {
        let elem = document.getElementById("rocket-button");
        setTimeout(() => {
          elem.classList.remove("animated");
          elem.classList.remove("live");
          elem.classList.remove("isSuccess");
...
.rocker-launch {
  ...
  * {
  ...
  .rocket-button {
    ...
    &.live {
      &:before {
        animation: shake 1.8s infinite ease;
      }
      .default {
        &:before {
          opacity: 0;
      ...
      .animation {
        .rocket {
          animation: rocket-ready 2s ease forwards 0.4s;
          &:before {
            animation: rocket-light 2s ease forwards 0.4s;
          }
        }
        .isSuccess {
          animation: rocket-success 1.5s ease forwards;
          &:before {
            animation: rocket-light 2s ease forwards 0.4s;
          }
        }
        .smoke {
          i {
            animation: smoke 1.7s ease infinite;
            &:nth-child(3),
            &:nth-child(6) {
              animation: smoke-alt 1.7s ease infinite;
            }
          }
        }
      }
